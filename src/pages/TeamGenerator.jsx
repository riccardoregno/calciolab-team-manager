...
function buildTeams(players, numTeams) {
  ...
  for (const p of players) groups[normalizeRoleToGroup(p.role)].push(p);
  const teams = Array.from({ length: numTeams }, () => []);
  for (const groupKey of GROUP_ORDER) {
    shuffle(groups[groupKey]).forEach((p, i) => teams[i % numTeams].push(p));
  }
  return teams;
}
...
function buildTrainingDraftFromTeams(teams) {
  const lines = teams.map((team, ti) => {
    const col = TEAM_COLORS[ti] || TEAM_COLORS[0];
    const names = team.map((p) => p.name).join(", ");
    return `${col.name} (${team.length}): ${names}`;
  });
  return {
    title: "Allenamento con suddivisione squadre",
    type: "Allenamento",
    notes: `Suddivisione squadre:\n${lines.join("\n")}`,
    exercises: [],
    attendance: {},
    teamSplit: teams.map((team, ti) => ({
      name: (TEAM_COLORS[ti] || TEAM_COLORS[0]).name,
      players: team.map((p) => ({ id: String(p.id), name: p.name, role: p.role || "" })),
    })),
  };
}

function PlayerRow({ p, isSelected, onToggle }) {
  return (
    <button
      type="button"
      ...
      style={{
        ...
        textAlign: "left", width: "100%", transition: "0.12s",
      }}
    >
      <span style={{
      ...
      {p.squadra === "juniores" && (
        <span style={{ fontSize: 9, fontWeight: 800, color: "#fb923c", background: "rgba(251,146,60,0.12)", border: "1px solid rgba(251,146,60,0.25)", borderRadius: 999, padding: "1px 5px" }}>JUN</span>
      )}
      {p.shirtNumber != null && (
...
export default function TeamGenerator({ players = [], teamId = null }) {
  const navigate = useNavigate();
  const isMobile = useIsMobile();

  // Carica disponibilità corrente da Supabase
  const [availMap, setAvailMap] = useState({});
  useEffect(() => {
    if (!teamId) return;
    fetchPlayerAvailability({ teamId }).then(({ data }) => {
      const map = {};
      for (const r of (data || [])) map[String(r.player_id)] = r.status;
      setAvailMap(map);
    });
  }, [teamId]);

  // Prima squadra: tutti tranne Infortunato/Squalificato
  // Juniores: solo quelli con availability "available"
  const availablePlayers = useMemo(() => {
    return players.filter((p) => {
      if (["Infortunato", "Squalificato"].includes(p.status || "")) return false;
      if (p.squadra === "juniores") return availMap[String(p.id)] === "available";
      return true;
    });
  }, [players, availMap]);

  // Juniores non disponibili (non "available") → sezione grigia
  const unavailableJuniores = useMemo(() =>
    players.filter((p) => p.squadra === "juniores" && availMap[String(p.id)] !== "available"),
  [players, availMap]);

  // Giocatori prima squadra non disponibili (inj/susp)
  const unavailablePrima = useMemo(() =>
    players.filter((p) => p.squadra !== "juniores" && ["Infortunato", "Squalificato"].includes(p.status || "")),
  [players]);
  ...
  // Quando availMap cambia (dopo fetch), aggiorna la selezione includendo i juniores disponibili
  useEffect(() => {
    if (Object.keys(availMap).length === 0) return;
    let cancelled = false;
  ...
  }, [availMap]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function createTraining() {
    if (!teams) return;
    const draft = buildTrainingDraftFromTeams(teams);
    try { sessionStorage.setItem("trainings_draft", JSON.stringify(draft)); } catch { /* ignore */ }
    navigate("/trainings", { state: { draftTraining: draft } });
  }

  // Raggruppa per reparto — separando prima e juniores per il pannello selezione
  const primaByGroup = useMemo(() => {
    const map = Object.fromEntries(GROUP_ORDER.map((g) => [g, []]));
    for (const p of availablePlayers.filter((p) => p.squadra !== "juniores")) {
      map[normalizeRoleToGroup(p.role)].push(p);
    }
    return map;
  }, [availablePlayers]);

  const junioresList = useMemo(() =>
    availablePlayers.filter((p) => p.squadra === "juniores"),
  [availablePlayers]);
  ...
  return (
    <div style={{ display: "grid", gap: 22 }}>
      <PageHeader
        ...
        badge={`${selectedCount} selezionati`}
      />
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(260px,320px) 1fr", gap: 18, alignItems: "start" }}>
        ...
        <AppCard>
          ...
          {/* Prima squadra — per reparto */}
          <p style={{ margin: "0 0 8px", fontSize: 10, fontWeight: 900, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em" }}>
            Prima squadra
          </p>
          {GROUP_ORDER.map((group) => {
            const gp = primaByGroup[group];
            if (!gp.length) return null;
            return (
              <div key={group} style={{ marginBottom: 12 }}>
                <p style={{ margin: "0 0 4px", fontSize: 10, fontWeight: 800, color: roleGroupColor(group), textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  {group}
                </p>
                <div style={{ display: "grid", gap: 3 }}>
                  {gp.map((p) => (
                    <PlayerRow key={p.id} p={p} isSelected={selected.has(String(p.id))} onToggle={togglePlayer} />
                  ))}
                </div>
              </div>
            );
          ...
          {/* Juniores aggregabili */}
          {junioresList.length > 0 && (
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid rgba(251,146,60,0.18)" }}>
              <p style={{ margin: "0 0 8px", fontSize: 10, fontWeight: 900, color: "#fb923c", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                Juniores · disponibili
              </p>
              <div style={{ display: "grid", gap: 3 }}>
                {junioresList.map((p) => (
                  <PlayerRow key={p.id} p={p} isSelected={selected.has(String(p.id))} onToggle={togglePlayer} />
          ...
          {/* Non disponibili */}
          {(unavailablePrima.length > 0 || unavailableJuniores.length > 0) && (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              <p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 800, color: "#475569", textTransform: "uppercase" }}>
                Non disponibili
              </p>
              {unavailablePrima.map((p) => (
                <div key={p.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0", opacity: 0.45 }}>
                  <span style={{ fontSize: 13, color: "#64748b" }}>{p.name}</span>
                  <Badge tone={p.status === "Infortunato" ? "red" : "orange"}>{p.status}</Badge>
                </div>
              ))}
              {unavailableJuniores.map((p) => (
                <div key={p.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0", opacity: 0.45 }}>
                  <span style={{ fontSize: 13, color: "#64748b" }}>{p.name}</span>
                  <span style={{ fontSize: 9, fontWeight: 800, color: "#fb923c", background: "rgba(251,146,60,0.10)", borderRadius: 999, padding: "1px 5px" }}>JUN</span>
                  <span style={{ fontSize: 11, color: "#475569" }}>non disponibile</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <AppCard>
            <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginLeft: "auto", flexWrap: "wrap" }}>
                {teams && (
                  <Button variant="ghost" onClick={generate} aria-label="Rimescola squadre">
                    🔀 Rimescola
                  </Button>
                )}
          ...
          {teams ? (
            <>
                          ...
                          <div key={group} style={{ marginBottom: 10 }}>
                            ...
                            {gp.map((p) => (
                              <div key={p.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                ...
                                {p.squadra === "juniores" && (
                                  <span style={{ fontSize: 9, fontWeight: 800, color: "#fb923c" }}>JUN</span>
                                )}
                              </div>
                            ))}
                          </div>
              ...
              {/* Bottone crea allenamento */}
              <AppCard>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 700 }}>Usa questa suddivisione</p>
                    <p style={{ margin: "2px 0 0", fontSize: 12, color: "#64748b" }}>
                      Crea un allenamento con le squadre già impostate nelle note
                    </p>
                  </div>
            ...
            <AppCard>
              <div style={{ textAlign: "center", padding: "32px 0", color: "#475569" }}>
                ...
                <p style={{ margin: "8px 0 0", fontSize: 13, color: "#475569" }}>
                  Prima squadra + Juniores disponibili, distribuiti per reparto.
                </p>
              </div>
            </AppCard>
          )}
